...
/// It is derived from the snip20 reference inmplementation (https://github.com/scrtlabs/snip20-reference-impl)
/// It adds the rebase mecanism on top of the usual token mecanisms
...
use crate::state::{
    ...
    write_viewing_key, GonBalances, Config, Constants, ReadonlyGonBalances, ReadonlyConfig,
};
...
pub const INITIAL_FRAGMENTS_SUPPLY: u128 = 5_000_000_u128* 10_u128.pow(9_u32);
pub const MAX_SUPPLY: u128 = u128::MAX; // (2^128) - 1
...
pub fn gons_for_balance(
    ...
    U256::from(amount).checked_mul(gons_per_fragment).ok_or_else(|| {
        StdError::generic_err("Opely, there is just too much zeros in this number")
    })
}
...
pub fn balance_of<S: Storage, A: Api, Q: Querier>(
    ...
    let gons_per_fragment = U256::from_dec_str(&ReadonlyConfig::from_storage(& deps.storage).constants().unwrap().gons_per_fragment).unwrap();
...
pub fn query_balance<S: Storage, A: Api, Q: Querier>(
    ...
    let amount = Uint128(balance_of(deps,account));
...
fn try_transfer_impl<S: Storage, A: Api, Q: Querier>(
    ...
    perform_transfer(&mut deps.storage, constants.clone(), &sender, &recipient, amount.u128())?;
...
fn try_transfer_from_impl<S: Storage, A: Api, Q: Querier>(
    ...
    perform_transfer(&mut deps.storage,  constants.clone(), owner, recipient, raw_amount)?;
...
fn perform_transfer<T: Storage>(
    ...
    let mut to_balance = gon_balances.gon_balance(to);
    to_balance = to_balance.checked_add(gon_value).ok_or_else(|| {
        StdError::generic_err("This tx will literally make them too rich. Try transferring less")
    })?;
    gon_balances.set_account_gon_balance(to, to_balance);
...
fn check_if_staking_contract<S: Storage>(config: &Config<S>, account: &HumanAddr) -> StdResult<()> {
    if !is_staking_contract(config, account)? {
        return Err(StdError::generic_err(
            "This is an admin command. Admin commands can only be run from admin address",
        ));
    }
