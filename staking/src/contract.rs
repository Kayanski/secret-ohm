...
use crate::msg::{
    ...
    QueryAnswer, QueryMsg, ReceiveMsg, ResponseStatus::Success, SOhmHandleMsg, SOhmQueryMsg,
    WarmupContractHandleMsg, DistributorQueryMsg, NextRewardForResponse
};
...
pub fn stake<S: Storage, A: Api, Q: Querier>(
    ...
    // Now we can stake the received OHM
    let mut config = Config::from_storage(&mut deps.storage);
    // First we need to store that we will stake some tokens later, to prevent the rebase to count those in
    config.is_currently_staking(amount);
    ...
    let gons_for_balance_query_msg = SOhmQueryMsg::GonsForBalance {
    ...
    let gons_deposited = U256::from_dec_str(&gons_for_balance_response.gons_for_balance.gons).unwrap();

    config.set_warmup_info(
        &canon_recipient,
        Claim {
            deposit: Uint128(
                claim_info
                    ...
                    .ok_or_else(|| {
                        StdError::generic_err(
                            "Sorry, can't deposit, the contract already contains too much sOHM",
                        )
                    })?,
            ),
            gons: U256::to_string(
                &U256::from_dec_str(&claim_info.gons)
                    .unwrap()
                    .checked_add(gons_deposited)
                    .ok_or_else(|| {
                        StdError::generic_err(
                            "Sorry, can't deposit, the contract already contains too much sOHM",
                        )
            ...
            expiry: consts
                .epoch
                .number
                .checked_add(consts.warmup_period)
                .ok_or_else(|| {
                    StdError::generic_err(
                        "Sorry, can't deposit, the maximum epoch has been reached",
                    )
                })?,
            lock: false,
        },
    ...
    //We will transfer sOHM to the warmup contract first
    let mut messages = vec![snip20::transfer_msg(
    ...
    // And then rebase if necessary
    messages.extend(rebase(deps, env)?.messages);
...
pub fn rebase<S: Storage, A: Api, Q: Querier>(
    ...
    let currently_staking = config.currently_staking();
    ...
    if consts.epoch.end_block <= env.block.height {
        ...
        messages.push(rebase_msg.to_cosmos_msg(
            consts.sohm.code_hash.clone(),
            consts.sohm.address.clone(),
            None,
        )?);

        // We update the epoch number (not much effect there)
        consts.epoch.end_block = consts.epoch.end_block + consts.epoch.length;
        consts.epoch.number += 1;

        // We register a distribute message, with the current distribute variable
        let distribute_msg = DistributorHandleMsg::Distribute {};
        messages.push(distribute_msg.to_cosmos_msg(
            distributor.code_hash.clone(),
            distributor.address.clone(),
            None,
        )?);

        // We update the distribute variable for the next epoch 
        // The distribute contract message, we just executed above has however side_effects on this action
        // We therefore anticipate on what the distributor will provide in terms of OHM
        let next_distributor_reward = {
            let next_distributor_reward_msg = DistributorQueryMsg::NextRewardFor { 
                recipient: env.contract.address 
            };
            let next_distributor_reward_response: NextRewardForResponse =
                next_distributor_reward_msg.query(
                    &deps.querier,
                    distributor.code_hash.clone(),
                    distributor.address.clone(),
                )?;
            next_distributor_reward_response
                .next_reward_for
                .amount
                .u128()
        };
        ...
        let balance = contract_balance(deps)? + next_distributor_reward;
        let changes_in_rebase = {
            //We need to ask the contract for the circulating_supply
            let changes_in_rebase_query_msg = SOhmQueryMsg::ChangesInRebase {profit:consts.epoch.distribute};
            let changes_in_rebase_response: ChangesInRebaseResponse =
                changes_in_rebase_query_msg.query(
                    &deps.querier,
                    consts.sohm.code_hash.clone(),
                    consts.sohm.address.clone(),
                )?;
            changes_in_rebase_response
                .changes_in_rebase
        };

        let staked = (changes_in_rebase.circulating_supply.u128() + currently_staking)
                *changes_in_rebase.total_supply_after.u128()/changes_in_rebase.total_supply_before.u128();
...
pub fn contract_balance<S: Storage, A: Api, Q: Querier>(deps: &Extern<S, A, Q>) -> StdResult<u128> {
    ...
    .amount
    .u128()
    .checked_add(constants.total_bonus.u128())
    .ok_or_else(|| StdError::generic_err("The contract is too rich for you, sorry"))
}

fn query_contract_balance<S: Storage, A: Api, Q: Querier>(deps: &Extern<S, A, Q>) -> QueryResult {
...
pub fn forfeit<S: Storage, A: Api, Q: Querier>(
    ...
    let balance_for_gons_response: BalanceForGonsResponse = balance_for_gons_query_msg
        .query(
            &deps.querier,
            config.constants()?.sohm.code_hash.clone(),
            config.constants()?.sohm.address.clone(),
...
pub fn toggle_deposit_lock<S: Storage, A: Api, Q: Querier>(
    ...
    Ok(HandleResponse {
        messages: vec![],
        log: vec![],
        data: Some(to_binary(&HandleAnswer::ToggleDepositLock {
            status: Success,
        })?),
    })
}
...
pub fn index<S: Storage, A: Api, Q: Querier>(deps: &Extern<S, A, Q>) -> StdResult<String> {
    let config = ReadonlyConfig::from_storage(&deps.storage);
    //We get the balance for gons equivalent
...
fn query_index<S: Storage, A: Api, Q: Querier>(deps: &Extern<S, A, Q>) -> QueryResult {
    let index = index(deps)?;
    to_binary(&QueryAnswer::Index { index: index })
}
...
pub fn give_lock_bonus<S: Storage, A: Api, Q: Querier>(
    ...
    constants.total_bonus = Uint128(
        constants
            ...
            .ok_or_else(|| {
                StdError::generic_err(
                    "Sorry, can't give bonus to the locker contract, too much bonus already",
                )
            })?,
    );
    config.set_constants(&constants)?;
...
pub fn return_lock_bonus<S: Storage, A: Api, Q: Querier>(
    ...
    constants.total_bonus = Uint128(
        constants
            ...
            .ok_or_else(|| {
                StdError::generic_err(
                    "Sorry, can't return bonus to the locker contract, nothing to give back :/",
                )
            })?,
    );
    config.set_constants(&constants)?;
    ...
    Ok(HandleResponse {
        messages: vec![],
        log: vec![],
        data: Some(to_binary(&HandleAnswer::ReturnLockBonus {
            status: Success,
        })?),
    })
}
...
pub fn set_contract<S: Storage, A: Api, Q: Querier>(
    ...
    match contract_type {
        ...
        ContractType::WarmupContract => {
            if contracts.warmup == Contract::default() {
                contracts.warmup = contract
            }
        }
        ContractType::Locker => {
            if contracts.locker == Contract::default() {
                contracts.locker = contract
            }
        }
    };
...
pub fn set_warmup_period<S: Storage, A: Api, Q: Querier>(
    ...
    Ok(HandleResponse {
        messages: vec![],
        log: vec![],
        data: Some(to_binary(&HandleAnswer::SetWarmupPeriod {
            status: Success,
        })?),
    })
}
...
mod tests {
    ...
    fn init_helper() -> (
        ...
        let init_msg = InitMsg {
            admin: None,
            ohm: Contract {
                address: HumanAddr("ohm".to_string()),
                code_hash: "Complicated_hash".to_string(),
            },
            sohm: Contract {
                address: HumanAddr("sohm".to_string()),
                code_hash: "Complicated_hash".to_string(),
            },
            epoch_length: 2200,
    ...
    fn test_stake() {
        ...
        let stake_msg = ReceiveMsg::Stake { recipient: None };
